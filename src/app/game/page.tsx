  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      const troggleCollision = gameState.troggles.some(
        (troggle) =>
          troggle.position.x === newPosition.x &&
          troggle.position.y === newPosition.y,
      );
    ...
    const moveTroggles = () => {
      setGameState((prev) => {
        ...
        const muncherCollision = newTroggles.some(
          (troggle) =>
            troggle.position.x === prev.muncher.x &&
            troggle.position.y === prev.muncher.y,
        );
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-blue-950 p-4 text-white">
      {gameState.gameStatus === "ready" && (
        <div className="absolute z-10 rounded-lg bg-blue-600 p-8 text-center shadow-xl">
          <h2 className="mb-4 text-2xl font-bold">Level {gameState.level}</h2>
          <Button className="h-16 w-40 text-2xl" onClick={startGame}>
            Start
          </Button>
        </div>
      )}
      ...
      {gameState.gameStatus === "gameOver" && (
        <div className="absolute z-10 rounded-lg bg-blue-600 p-8 text-center shadow-xl">
          ...
          <Button
            className="mb-2 h-12 w-40 text-xl"
            onClick={() => router.push("/")}
          >
            Main Menu
          </Button>
      ...
      <div className="mx-auto w-full max-w-3xl">
        <GameHeader
          ...
          rule={gameState.rule}
          ruleValue={gameState.ruleValue}
        />

        <GameGrid
          grid={gameState.grid}
          muncher={gameState.muncher}
          troggles={gameState.troggles}
        />
